...
def train_test() :
    ...
    maxTrEg=1500 # maximum number of pos & neg training examples, increased the number of training examples
    maxTeEg=40# maximum number of pos & neg test examples

    # read the data
    train_x, train_y = readPosNeg("aclImdb/train/pos","aclImdb/train/neg",maxTrEg)
    test_x, test_y = readPosNeg("smalldataset/test/pos","smalldataset/test/neg",maxTeEg)
    ...
    model.fit([tokenized_train["input_ids"],tokenized_train["attention_mask"]],
              train_y, batch_size=25, epochs=4)

    # evaluate
    score = model.evaluate([tokenized_test["input_ids"],tokenized_test["attention_mask"]],test_y,verbose=0)
    
    print("Accuracy on test data:",score[1])
    a_test = model.predict([tokenized_test["input_ids"], tokenized_test["attention_mask"]]) 
    print(a_test.shape)
    print(a_test)
    for i in range(len(a_test)):
        print("The text file is given below,\n","\n", test_x[i],"\nPrediction of given review:", a_test[i], "\n")
